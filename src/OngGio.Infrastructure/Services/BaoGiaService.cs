...
public class BaoGiaService : IBaoGiaService
{
    ...
    public async Task<byte[]> ExportExcelAsync(int id, CancellationToken ct = default)
    {
        var baoGia = await GetByIdAsync(id, ct)
            ?? throw new KeyNotFoundException($"Không tìm thấy báo giá id={id}");

        using var workbook = new XLWorkbook();
        var sheet = workbook.Worksheets.Add("BaoGia");

        sheet.Cell(1, 1).Value = "Mã báo giá";
        sheet.Cell(1, 2).Value = baoGia.MaBaoGia;
        sheet.Cell(2, 1).Value = "Khách hàng";
        sheet.Cell(2, 2).Value = baoGia.TenKhachHang;
        sheet.Cell(3, 1).Value = "Trạng thái";
        sheet.Cell(3, 2).Value = baoGia.TrangThai;
        sheet.Cell(4, 1).Value = "Ngày tạo";
        sheet.Cell(4, 2).Value = baoGia.NgayTao.ToString("dd/MM/yyyy HH:mm");

        var headers = new[]
        {
            "STT", "Sản phẩm", "Loại tôn", "W (mm)", "H (mm)", "SL",
            "Diện tích SX/cái (m²)", "Tổng diện tích (m²)", "Trọng lượng (kg)",
            "Thành tiền tôn", "Đơn giá", "Thành tiền"
        };
        ...
        foreach (var line in baoGia.ChiTietBaoGias)
        {
            ...
            sheet.Cell(row, 2).Value = line.NhomSanPham?.TenNhom ?? "";
            sheet.Cell(row, 3).Value = $"{line.LoaiTon?.ThuongHieu} {line.LoaiTon?.DoDay}mm";
            sheet.Cell(row, 4).Value = line.WInput;
            sheet.Cell(row, 5).Value = line.HInput;
            sheet.Cell(row, 6).Value = line.SoLuong;
            sheet.Cell(row, 7).Value = line.DienTichSx1Cai;
            sheet.Cell(row, 8).Value = line.TongDienTichLo;
            sheet.Cell(row, 9).Value = line.TrongLuongKg;
            sheet.Cell(row, 10).Value = line.ThanhTienTon;
            sheet.Cell(row, 11).Value = line.DonGiaCuoi;
            sheet.Cell(row, 12).Value = line.ThanhTien;
            row++;
        }

        sheet.Cell(row + 1, 10).Value = "Tổng trước thuế:";
        sheet.Cell(row + 1, 12).Value = baoGia.TongTienTruocThue;
        sheet.Cell(row + 2, 10).Value = "Tổng sau thuế:";
        sheet.Cell(row + 2, 12).Value = baoGia.TongTienSauThue;
    ...
    private async Task ApplyLinesAsync(BaoGia baoGia, CreateBaoGiaRequest request, CancellationToken ct)
    {
        ...
        foreach (var line in request.Lines)
        {
            var calcRequest = new CalculationRequest(
                line.NhomSanPhamId, line.LoaiTonId,
                line.W, line.H, line.SoLuong,
                line.GiaNhanCong, line.PhuKien);
        ...
        baoGia.TongTienSauThue = tongTruocThue * (1 + request.ThueSuat);
    }
        ...
        int nhomSanPhamId, int loaiTonId, CancellationToken ct)
    {
        var nhom = await _db.NhomSanPhams.FirstOrDefaultAsync(x => x.Id == nhomSanPhamId, ct)
            ?? throw new KeyNotFoundException($"Không tìm thấy nhóm sản phẩm id={nhomSanPhamId}");
        var loaiTon = await _db.LoaiTons.FirstOrDefaultAsync(x => x.Id == loaiTonId, ct)
            ?? throw new KeyNotFoundException($"Không tìm thấy loại tôn id={loaiTonId}");
